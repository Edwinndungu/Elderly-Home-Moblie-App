package com.example.nyumbayawazeehomecare.Staff.Doctors.Adaptor;
...
import com.example.nyumbayawazeehomecare.Staff.Doctors.Model.LabTestModel;
import com.example.nyumbayawazeehomecare.Staff.Doctors.SendReport;

import java.util.ArrayList;
import java.util.List;

public class AdaptorLabTest extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    private List<LabTestModel> list,searchItems;
     private Context cnxt;
    private  Intent in;

    public AdaptorLabTest(Context context, List<LabTestModel> list) {
        this.list = list;
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof  OriginalViewHolder){
            ...
            if(data.getTestStatus().equals("Test done")){
                view.txv_date.setText("Appointment date: "
                        +data.getAppointmentDate()+"\n"+data.getRemarks()+"\nResult: "+data.getTestResults());
                view.img_view.setVisibility(View.VISIBLE);
            }
            view.img_view.setOnClickListener(v->{

                if(data.getTestStatus().equals("Test done")){
                    in=new Intent(cnxt, SendReport.class);
                }else{
//                    in=new Intent(cnxt, TestResults.class);
                }
                ...
                in.putExtra("remarks",data.getRemarks());
                in.putExtra("results",data.getTestResults());
