...
import static com.example.nyumbayawazeehomecare.ConstFiles.Urls.URL_GET_DONATIONS;
...
import com.example.nyumbayawazeehomecare.Staff.StoreManager.Adaptor.AdaptorDonationList;
import com.example.nyumbayawazeehomecare.Staff.StoreManager.Model.DonationListModel;
...
public class NewDonations extends AppCompatActivity {

    private ProgressBar progressBar;
    private RecyclerView recyclerView;
    private List<DonationListModel> list;
    private AdaptorDonationList adaptor;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Intent in=getIntent();
        donationStatus=in.getStringExtra("donationStatus");
    ...
    private void donations(){
        ...
        StringRequest stringRequest=new StringRequest(Request.Method.POST, URL_GET_DONATIONS,
                response -> {
                    ...
                    try {
                        ...
                        if(status.equals("1")){
                            ...
                            for(int i =0; i<jsonArray.length();i++){
                                ...
                                DonationListModel model=new DonationListModel( donationID,  name,  donationDate,
                                        donationStatus,  donationRemarks);
                                list.add(model);
                            }
                            adaptor=new AdaptorDonationList(getApplicationContext(),list);
